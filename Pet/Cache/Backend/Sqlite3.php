<?php
/**
 * Rewrite of the original Zend_Cache_Backend wich uses the much slower pdo_sqlite interface
 * This one uses the sqlite3 extension that has to be loaded
 * Be warned of unknown side effects although it is in productive use on many of our sites
 * Thanks to m.welt
 */
...
class Pet_Cache_Backend_Sqlite3 extends Zend_Cache_Backend implements Zend_Cache_Backend_ExtendedInterface
{
    /**
     * Available options
     *
     ...
     * DB Class
     *
     * @var SQLite3 $_objSqlite3
     */
    private $_objSqlite3 = null;
    ...
    {
        if($this->_objSqlite3 instanceof SQLite3){
            $this->_objSqlite3->close();
        }
    }
    ...
    public function save($data, $id, $tags = array(), $specificLifetime = false)
    {
        ...
        $data = $this->_getConnection()->escapeString($data);
        $mktime = time();
        if ($lifetime === null) {
            $expire = 0;
        } else {
        ...
        $this->_exec("DELETE FROM cache WHERE id='$id'");
    ...
    public function remove($id)
    {
        ...
        // im original code steht folgendes:
        // $result1 = @sqlite_fetch_single($res);
        // d.h. die überprüfen ob werte in der DB vorhanden sind, und geben dann entsprechend true oder false zurück
        // das '@' unterdrückt die fehlermeldung wenn es keine resultsetz gibt und $result1 wird false
        // wir umgehen das anders
        $result1 = false;
        if($res){
            $result1 = $res->fetchArray(SQLITE3_ASSOC);
            $res->finalize();
        }
        $result2 = $this->_exec("DELETE FROM cache WHERE id='$id'");
        $result3 = $this->_exec("DELETE FROM tag WHERE id='$id'");
    ...
    public function getIds()
    {
        ...
        $result = array();
        while ($id = $res->fetchArray(SQLITE3_ASSOC)) {
            $result[] = $id;
        }
        $res->finalize();
        return $result;
    ...
    public function getIdsNotMatchingTags($tags = array())
    {
        ...
        while($row = $res->fetchArray(SQLITE3_ASSOC)) {
            ...
            $matching = false;
            foreach ($tags as $tag) {
                $res1 = $this->_query("SELECT COUNT(*) AS nbr FROM tag WHERE name='$tag' AND id='$id'");
                if (!$res1) {
                    return array();
                }
    ...
    public function getMetadatas($id)
    {
        ...
        $res->finalize();
        $this->_exec('CREATE TABLE cache (id TEXT PRIMARY KEY, content BLOB, lastModified INTEGER, expire INTEGER)');
    ...
    public function touch($id, $extraLifetime)
    {
        ...
        $arrExpire = $res->fetchArray(SQLITE3_ASSOC);
        $newExpire = $arrExpire['expire'] + $extraLifetime;
        $res->finalize();
    ...
    {
        $time = time() - 1;
        $this->_exec("UPDATE cache SET lastModified=$time, expire=$time WHERE id='$id'");
    }
    ...
    {
        if ($this->_objSqlite3 instanceof SQLite3) {
            return $this->_objSqlite3;
        } else {
            //$this->_db = @sqlite_open($this->_options['cache_db_complete_path']);
            $this->_objSqlite3 = new SQLite3($this->_options['cache_db_complete_path']);
            if (! $this->_objSqlite3) {
    ...
    /**
     * Execute an SQL query and return resultset
     *
     * @param string $query SQL query
     * @return SQLite3Result|false query results
     */
    private function _query($query)
    {
        $objSqlite3 = $this->_getConnection();
        if ($objSqlite3) {
            $res = @$objSqlite3->query($query);
            if ($res === false) {
                return false;
            } else {
                return $res;
    ...
    private function _registerTag($id, $tag) {
        $this->_exec("DELETE FROM tag WHERE name='$tag' AND id='$id'");
        $res = $this->_exec("INSERT INTO tag (name, id) VALUES ('$tag', '$id')");
        if (!$res) {
    ...
    private function _buildStructure()
    {
        ...
        $this->_exec('DROP INDEX tag_name_index');
        $this->_exec('DROP INDEX cache_id_expire_index');
        $this->_exec('DROP TABLE version');
        $this->_exec('DROP TABLE cache');
        $this->_exec('DROP TABLE tag');
        $this->_exec('CREATE TABLE version (num INTEGER PRIMARY KEY)');
        $this->_exec('CREATE TABLE cache (id TEXT PRIMARY KEY, content BLOB, lastModified INTEGER, expire INTEGER)');
        $this->_exec('CREATE TABLE tag (name TEXT, id TEXT)');
        $this->_exec('CREATE INDEX tag_id_index ON tag(id)');
        $this->_exec('CREATE INDEX tag_name_index ON tag(name)');
        $this->_exec('CREATE INDEX cache_id_expire_index ON cache(id, expire)');
        $this->_exec('INSERT INTO version (num) VALUES (3)');
    }
    ...
    private function _clean($mode = Zend_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        switch ($mode) {
            case Zend_Cache::CLEANING_MODE_ALL:
                $res1 = $this->_exec('DELETE FROM cache');
                $res2 = $this->_exec('DELETE FROM tag');
                return $res1 && $res2;
                break;
            case Zend_Cache::CLEANING_MODE_OLD:
                $mktime = time();
                $res1 = $this->_exec("DELETE FROM tag WHERE id IN (SELECT id FROM cache WHERE expire>0 AND expire<=$mktime)");
                $res2 = $this->_exec("DELETE FROM cache WHERE expire>0 AND expire<=$mktime");
                return $res1 && $res2;
                break;
